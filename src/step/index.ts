...
import type { StepRequirements } from "./services.ts";
...
export function step(
  ...
  signal?: AbortSignal,
): Effect.Effect<StepResult, Error, StepRequirements> {
  return Effect.gen(function* () {
    const terminal = yield* TerminalService;
    let history = conversation.history;

    while (true) {
      // Check for abort signal before each provider call
      if (signal?.aborted) {
      ...
      const completion = yield* sessionConfiguration.provider.complete(
        ...
        sessionConfiguration.loadedTools.map((loaded) => loaded.tool),
        signal,
      );
...
function modelMessageFromCompletion(
  ...
  return {
    ...
    ...(completion.roundTripContext !== undefined
      ? { roundTripContext: completion.roundTripContext }
      : {}),
  };
}
...
function runToolCall(
  ...
  loadedTools: readonly LoadedTool[],
): Effect.Effect<string, never, StepRequirements> {
  return Effect.gen(function* () {
    const terminal = yield* TerminalService;
    ...
    const validationResult = yield* Effect.either(
      Effect.try({
        try: () => {
          loaded.validateInput(call.input);
        },
        catch: toError,
      }),
    );
    if (validationResult._tag === "Left") {
      terminal.show({
        ...
        status: {
          type: "toolCall",
          text: `Schema validation error: ${validationResult.left.message}`,
        },
      });
    ...
    terminal.show({
      ...
      status: {
        type: "toolCall",
        text: loaded.tool.getCallStatusMessage(call.input),
      },
    });

    const runResult = yield* Effect.either(
      Effect.tryPromise({
        try: () => loaded.tool.run(call.input),
        catch: toError,
      }),
    );
    if (runResult._tag === "Left") {
      terminal.show({
        ...
        status: {
          type: "toolCall",
          text: `Error: ${runResult.left.message}`,
        },
      });
