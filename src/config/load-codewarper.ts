...
const CONFIG_FILENAME = "codewarper.ts";
...
export interface CodewarperConfigLoader {
  ...
  load(): Effect.Effect<CodewarperConfig, Error>;
}
...
export function createCodewarperConfigLoader(environment: Environment): CodewarperConfigLoader {
  ...
  const loadAndRemember = () =>
    loadCodewarperConfigFromPath(configPath()).pipe(
...
export function loadCodewarperConfigFromPath(filePath: string): Effect.Effect<CodewarperConfig, Error> {
  return Effect.tryPromise({
    try: async () => {
      ...
      if (!def || typeof def !== "object") {
        throw new Error(`Codewarper config at ${filePath} must default-export an object.`);
      }

      const toolsUnknown = (def as { tools?: unknown }).tools;
      if (typeof toolsUnknown !== "undefined" && !Array.isArray(toolsUnknown)) {
        throw new Error(`Codewarper config at ${filePath} must use tools: Tool[] when provided.`);
      }
      ...
      if (typeof systemPromptUnknown !== "undefined" && typeof systemPromptUnknown !== "string") {
        throw new Error(`Codewarper config at ${filePath} must use a string for systemPrompt when provided.`);
      }
