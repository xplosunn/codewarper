...
export function refreshOpenAICodexAuth(
  ...
  return Effect.gen(function* () {
    ...
    const { accessToken, refreshToken, expiresIn, idToken } = yield* parseTokenResponse(
      response,
...
export function loginOpenAICodex(
  ...
  return Effect.gen(function* () {
    ...
    runtime.terminal.show({
      type: "system",
      text: "Trying to open your browser now. If nothing happens, open the URL manually.",
    });
    ...
    const winner = yield* fromPromise(() => {
      const callbackWinner: Promise<{ source: "callback"; result: OAuthCallbackCode | null }> =
        callbackPromise.then((result) => ({ source: "callback", result }));
...
function generatePkce(crypto: Crypto): { verifier: string; challenge: string } {
  return crypto.createPkcePair();
}

function createState(crypto: Crypto): string {
  return crypto.createRandomHex(16);
}
...
function extractAccountId(crypto: Crypto, accessToken: string, idToken: string | null): string {
  ...
  if (accountId === null) {
    accountId = readAccountIdFromToken(crypto, accessToken);
  }

  if (!accountId) {
    throw new Error("Failed to extract ChatGPT account ID from login tokens.");
  }
  return accountId;
}
...
function parseAuthorizationInput(input: string): { code: string | null; state: string | null } {
  ...
  if (value.includes("code=")) {
    ...
    return {
      code: readSearchParam(params, "code"),
...
function parseTokenResponse(
  ...
  { accessToken: string; refreshToken: string; expiresIn: number; idToken: string | null },
  Error
> {
  return Effect.gen(function* () {
    if (!response.ok) {
...
function readManualAuthorizationCode(
  terminal: Terminal,
