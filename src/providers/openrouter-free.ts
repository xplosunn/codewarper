...
const listOptions = (auth: ProviderAuth): Effect.Effect<ProviderOption[], Error, HttpClientService> =>
  Effect.gen(function* () {
    const models = yield* fetchModels(auth);

    const choices = models.map((model) => ({ id: model.id, name: model.name }));
...
const fetchModels = (auth: ProviderAuth): Effect.Effect<OpenRouterModel[], Error, HttpClientService> =>
  Effect.gen(function* () {
