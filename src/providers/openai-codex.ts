...
const ensureAuthenticated = (forceLogin: boolean) =>
  Effect.gen(function* () {
...
function complete(
  ...
  tools: readonly Tool[],
  signal?: AbortSignal,
): Effect.Effect<ProviderCompletion, Error, HttpClientService | SystemInfoService | TerminalService> {
...
const listOptions = (_auth: ProviderAuth): Effect.Effect<ProviderOption[]> => Effect.succeed(OPTIONS);
...
const collectStreamedCompletion = (response: Response, signal?: AbortSignal): Effect.Effect<ProviderCompletion, Error> =>
  Effect.gen(function* () {
...
const parseError = (response: Response): Effect.Effect<string, Error> =>
  Effect.gen(function* () {
...
const loginAndSaveAuth = (authStore: ProviderAuthStore, loginRuntime: { terminal: Terminal; http: HttpClient; clock: Clock; oauth: OAuth; crypto: Crypto }): Effect.Effect<ProviderAuth, Error> =>
  Effect.gen(function* () {
...
const refreshAndSaveAuth = (authStore: ProviderAuthStore, refreshRuntime: { http: HttpClient; clock: Clock; crypto: Crypto }, auth: ProviderAuth): Effect.Effect<ProviderAuth, Error> =>
  Effect.gen(function* () {
